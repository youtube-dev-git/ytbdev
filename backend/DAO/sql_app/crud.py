...
def create_admin(db: Session, admin: schemas.AdminCreate):
    ...
    db_admin = models.Admin(email=admin.email, hashed_password=fake_hashed_password, name =admin.name, photo=admin.photo, phone_num=admin.phone_num,gender=admin.gender,statut=admin.statut)
    db.add(db_admin)
    db.commit()
    db.refresh(db_admin)
    return db_admin
...
def create_expert(db: Session, expert: schemas.ExpertCreate):
    ...
    db_expert = models.Expert(email=expert.email, hashed_password=fake_hashed_password, name =expert.name, photo=expert.photo, phone_num=expert.phone_num,gender=expert.gender,statut=expert.statut)
    db.add(db_expert)
    db.commit()
