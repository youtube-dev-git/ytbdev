<template>
<div class="login">
    <div class="login_img">
        <h1>Youtube Dev</h1>
      <img src="../../src/assets/login.png" alt="">
     </div>
    <div class="container">
     <div class="login-title">
        <h1>Se Connecter</h1>
     </div>

         <form @submit="login">
        <div class="login-control ">
          <input type="text" v-model="email" id="email" required>
          <label>Email</label>
        </div>
        <div class="login-control">
          <input type="password" v-model="password" id="password" required >
          <label>Password</label>
        </div>
        <button class="btn-login">Se connecter</button>
        <p class="login-txt test">Aucun compte ? 
        <span><router-link :to="{name: 'Register'}" class="link">S'inscrire</router-link></span></p>
    </form>
    </div>

</div>

</template>

<script>
import axios from 'axios'
export default {
    created(){
        axios.get('https://ng70hk.deta.dev/syllabus')
        .then(response =>{
         this.api_test = response.data
         console.log(this.api_test) 
     }) ,
    axios.post('https://ng70hk.deta.dev/login', {
        email: 'Fred',
        password: 'Flintstone'
    })
    .then(function (response) {
        console.log(response);
    })
    .catch(function (error) {
        console.log(error);
    });

    },
    data () {
      return {
        email:null,
        password:null,
        taille: '',
        sylab_tab:[],
        api_test:[],
      }
    },
    methods: {  
        showError: function(input, message){
            const formcontrol = input.parentElement;
            formcontrol.className = 'login-control error'; 
            const small = formcontrol.querySelector('small');
            small.innerText = message;
        },
    login(e){

        if(this.email && this.password){
            this.$router.push({name: 'Home_user', params: {pseudo:this.email }})
            alert('Connexion reussi')
       
        }        
        e.preventDefault()
    }
      
    },

}
</script>


<style lang="scss">

.login{
  position: relative;
  background-color: steelblue;
  border-radius: 5px;
  color: white;
  font-family: Arial, Helvetica, sans-serif;
  height: 100vh;width: 100%;
  display: flex;
  justify-content: center;
  align-items: center;
//   flex-direction: column;
  overflow: hidden;
  margin: 0;
}
.login_img{
    margin-right: 250px;
}
.login_img img{
width: 350px;
}
.login_img h1{
    text-align: center;
    font-size: 45px;
    text-transform: uppercase;
    font-weight: 700;
}
.login-title{
    font-weight: 500;
    font-size: 15px;
}
.login-txt{
    text-align: center;
}
.container{
    background-color: rgba(0, 0, 0, 0.4);
    padding: 20px 40px;
    border-radius: 5px;
}
.container h1{
    text-align: center;
}
.container a{
    text-decoration: none;
    color: lightblue;
}
 .login-control {  
  position: relative;  
  margin: 35px 0 40px;  
  width: 300px;  
 }  
 .login-control input {  
  background-color: transparent;  
  border: 0;  
  border-bottom: 2px #fff solid;  
  display: block;  
  width: 100%;  
  padding: 12px 0;  
  font-size: 18px;  
  color: #fff;   
 }  
 .login-control input:focus,  
 .login-control input:valid {  
  outline: 0;  
  border-bottom-color: lightblue;  
 }  
 .login-control label {  
  position: absolute;  
  top: 20px;  
  left: 0;  
  pointer-events: none;  
 }  
 .login-control label  {  
  display: inline-block;  
  font-size: 20px;  
  min-width: 5px;   
 }  
 .login-control input:valid + label ,  
 .login-control input:focus + label  {  
  color: lightblue;  
  transform: translateY(-33px);  
 }

.btn-login{
    cursor: pointer;
    display: inline-block;
    width: 100%;
    background: lightblue;
    padding: 10px;
    color: rgb(48, 46, 46);
    font-size: 16px;
    border: 0;
    border-radius: 5px;
}
.btn-login:focus{
    outline: 0;
}
.btn-login:hover{
    transform: scale(1.1);
}
</style>




